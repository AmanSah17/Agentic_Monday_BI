...
export const RegisterPage: React.FC<{ onToggle: () => void }> = ({ onToggle }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginResp.ok) {
        ...
        onToggle(); // Go back to login if auto-login fails
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0f] p-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-secondary-container/20 rounded-full blur-[120px]" />
      ...
      >
        <div className="bg-primary p-8 text-white text-center">
          <h1 className="text-3xl font-black tracking-tighter uppercase italic">Join the Future</h1>
          <p className="mt-3 text-sm text-primary-content/90 font-medium leading-relaxed">
            Unleash the power of AI-driven Business Intelligence directly from your Monday.com boards.
          </p>
        </div>
